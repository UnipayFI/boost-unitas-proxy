...
/**
 * @title Unitas Minting
 * @notice This contract mints and redeems USDu in a single, atomic, trustless transaction
 */
contract UnitasMintingV2 is IUnitasMintingV2, SingleAdminAccessControl, ReentrancyGuard {
  ...
  /// @notice EIP712 name
  bytes32 private constant EIP_712_NAME = keccak256("UnitasMinting");
  ...
  /// @notice running total usdu minted/redeemed per single block
  mapping(uint256 => BlockTotals) public totalPerBlock;

  /// @notice total usdu that can be minted/redeemed across all assets per single block.
  ...
  /// @notice ensure that the already minted usdu in the actual block plus the amount to be minted is below the maximum mint amount
  /// @param mintAmount The usdu amount to be minted
  ...
  /// @notice ensure that the already redeemed usdu in the actual block plus the amount to be redeemed is below the maximum redeem amount
  /// @param redeemAmount The usdu amount to be redeemed
  ...
  /// @notice ensure that the global, overall minted usdu in the actual block
  ...
  /// @notice ensure that the global, overall redeemed usdu in the actual block
  ...
  constructor(
    ...
    if (address(_usdu) == address(0)) revert InvalidUSDuAddress();
    ...
    // Set the global max usdu mint/redeem limits
    ...
    belowGlobalMaxRedeemPerBlock(order.usdu_amount)
  {
    ...
    totalPerBlockPerAsset[block.number][order.collateral_asset].redeemedPerBlock += order.usdu_amount;
    totalPerBlock[block.number].redeemedPerBlock += order.usdu_amount;
    usdu.burnFrom(order.benefactor, order.usdu_amount);
  ...
  /// @notice Sets the overall, global maximum usdu mint size per block
  ...
  /// @notice Sets the overall, global maximum usdu redeem size per block
  ...
  function addCustodianAddress(address custodian) public onlyRole(DEFAULT_ADMIN_ROLE) {
    if (custodian == address(0) || custodian == address(usdu) || !_custodianAddresses.add(custodian)) {
  ...
  function verifyOrder(
    ...
    if (typeOfToken == TokenType.STABLE) {
      if (!verifyStablesLimit(order.collateral_amount, order.usdu_amount, order.collateral_asset, order.order_type)) {
        revert InvalidStablePrice();
      }
    }
    if (order.beneficiary == address(0)) revert InvalidAddress();
    if (order.collateral_amount == 0 || order.usdu_amount == 0) revert InvalidAmount();
  ...
  function verifyStablesLimit(
    ...
    uint128 usduDecimals = _getDecimals(address(usdu));
    uint128 collateralDecimals = _getDecimals(collateralAsset);

    uint128 normalizedCollateralAmount;
    uint128 scale = uint128(
      usduDecimals > collateralDecimals
        ? 10 ** (usduDecimals - collateralDecimals)
    ...
    uint128 difference = normalizedCollateralAmount > usduAmount
      ...
      : usduAmount - normalizedCollateralAmount;

    uint128 differenceInBps = (difference * STABLES_RATIO_MULTIPLIER) / usduAmount;

    if (orderType == OrderType.MINT) {
      return usduAmount > normalizedCollateralAmount ? differenceInBps <= stablesDeltaLimit : true;
    } else {
      return normalizedCollateralAmount > usduAmount ? differenceInBps <= stablesDeltaLimit : true;
    }
  }
  ...
  function addSupportedAsset(address asset, TokenConfig memory _tokenConfig) external onlyRole(DEFAULT_ADMIN_ROLE) {
    if (tokenConfig[asset].isActive || asset == address(0) || asset == address(usdu)) {
      revert InvalidAssetAddress();
    }
